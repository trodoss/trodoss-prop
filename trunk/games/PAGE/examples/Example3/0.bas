...
Room ROOM_OLD_SHED
	...
	'Room backdrop
	'-----------------
	Dim backBackdrop as Backdrop ("old_shed.bmp")
	
	'-----------------
	'Room hotspot(s)
	'-----------------
	Dim hotspot1 as Hotspot (32, 66, 10, 4)
	Dim hotspot2 as Hotspot (0, 60, 4, 19)
	Dim hotRake as Hotspot (10, 62, 10, 17)
	
	'-----------------
	'Room pic(s)
	'-----------------
	Dim picRake as Pic ("rake.bmp")

	'-----------------
	'On_Load event
	'-----------------   
	Event On_Load	
		'Draw the rake by the shed, if the rake
		'has not been taken
